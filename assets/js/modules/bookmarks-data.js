...
export const BookmarksAPI = {
    ...
    return [
      {
        ...
        description: 'A short list by design, not a dumping ground.'
      },
      {
        label: 'Buckets',
        value: String(Object.keys(BOOKMARKS_DATA).length),
        description: 'Articles, tools, learning resources, and inspiration.'
      }
    ];
  },
  ...
  renderEmptyState(label) {
    return `
      <div class="collection-empty-state compact-empty-state">
        <p class="collection-empty-label">Empty for now</p>
        <h3>${label}</h3>
        <p>I have not added any links here yet because there is nothing worth keeping publicly on this list.</p>
      </div>`;
  }
};
...
export function generateBookmarksSections() {
  const sections = Object.entries({
    'Articles': BOOKMARKS_DATA.articles,
    'Tools': BOOKMARKS_DATA.tools,
    'Learning': BOOKMARKS_DATA.learning,
    'Inspiration': BOOKMARKS_DATA.inspiration
  });

  return sections.map(([heading, items]) => `
    <section class="section collection-section" id="${heading.toLowerCase()}">
      ...
      ${items.length > 0
        ? `<div class="bookmarks-grid">${items.map(item => BookmarksAPI.createCardTemplate(item)).join('')}</div>`
        : BookmarksAPI.renderEmptyState(heading)}
    </section>`).join('');
}
